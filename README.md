...
пакет; `mypy` гейтит расчётное ядро + сериализацию (33 модуля, см.
`[tool.mypy] files`). Вне гейта пока `io_excel`/`io_pdf` и UI-слой —
подключаются по мере очистки. Оба прогона блокирующие в CI
(job **Lint & types**).
