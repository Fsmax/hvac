    ...
    for row in raw_rows:
        ...
        if row_type in ("external_wall", "opening"):
            if wet_curtain_artifact:
                ...
                # Если ВСЕ остальные соседи — неотапливаемые (балконы/шахты),
                # элемент граничит с «улицей» → наружный. Если есть хотя бы
                # один ОТАПЛИВАЕМЫЙ сосед — обе стороны тёплые, это
                # межкомнатная перегородка → ВНУТРЕННЯЯ.
                #
                # Это касается и витража: стеклянная перегородка между
                # комнатами номера (ванная↔гостиная, гостиная↔спальня) —
                # частый случай в гостиницах/квартирах — НЕ фасад. Реальное
                # фасадное остекление выгружается отдельными элементами с
                # bsc=1 (ветка выше), они остаются наружными.
