...
Источники
---------
ШНК 2.04.05-22 «Иситиш, вентиляция ва кондициялаш» (Отопление, вентиляция
и кондиционирование), Прил. 20 (20-ИЛОВА) — в СНиП 2.04.05-91* это Прил. 22:
    - формула (1): G = 3420·B·n·H^1.5         (коридор/холл, жилые)
    - формула (2): G = 4300·B·n·H^1.5·Kd      (коридор/холл, общественные)
    - формула (3): G = 676.8·Pf·y^1.5·Ks      (помещение ≤ 1600 м²)
    - формула (4): Pf = 0.38·A^0.5 ≤ 12       (периметр очага, если неизвестен)
      где B — ширина створки двери, H — высота двери (≤2.5 м),
      n — табличный коэф. от B, Kd — коэф. продолжительности открывания.

ШНК 2.04.05-22 §7 «Ёнғиндаги тутунга қарши ҳимоя» (пп. 323–341) —
приточная противодымная вентиляция (подпор воздуха):
    - п. 340: расход подпора при одной открытой двери определяется
      расчётом или по скорости 1,3 м/с в проёме двери: L = 3600·v·F·n;
    - п. 341: избыточное давление ≥ 20 Па; на закрытых дверях путей
      эвакуации ≤ 150 Па; для двупольной двери — площадь большей створки;
    - п. 327, ф.(5): подсос через закрытые клапаны Gv = 40.3·(Av·ΔP)^0.5·n;
    - п. 332: температура/уд. вес дыма (коридоры t=300 °C, γ=6 Н/м³).
...
def corridor_n_coefficient(door_width_m: float, is_public: bool = False) -> float:
    """Коэффициент n по табл. ШНК 2.04.05-22 Прил. 20 (СНиП 2.04.05-91* — Прил. 22).
...
def kmk_corridor_kg_h(door_width_m: float, door_height_m: float = 2.0,
    ...
    """Расход дыма из коридора/холла, кг/ч — ШНК 2.04.05-22 Прил. 20.
    ...
    Коэффициент n берётся из таблицы Прил. 20 по ширине B (см.
...
# ---------------------------------------------------------------------------
#  ШНК 2.04.05-22 §7 — приточная противодымная вентиляция (подпор воздуха)
# ---------------------------------------------------------------------------

def fire_perimeter_from_area(area_m2: float) -> float:
    """ШНК 2.04.05-22, Прил. 20, ф.(4): периметр очага пожара, если его
    невозможно определить явно:  Pf = 0.38·A^0.5,  но не более 12 м."""
    if area_m2 <= 0:
        return 0.0
    return min(0.38 * (area_m2 ** 0.5), 12.0)


def pressurization_open_door_m3h(door_area_m2: float,
                                 v_door_m_s: float = 1.3,
                                 n_open_doors: int = 1) -> float:
    """Расход приточного воздуха на подпор по скорости в открытой двери.

    ШНК 2.04.05-22, п. 340: расход воздуха, подаваемого в тамбур-шлюз
    (незадымляемую лестничную клетку Н2, шахту лифта) при работе с одной
    открытой дверью на этаже пожара, определяется расчётом или по скорости
    1,3 м/с в проёме открытой двери:

        L = 3600 · v · F_door · n      [м³/ч]

    Параметры
    ---------
    door_area_m2 : F_door — площадь проёма открытой двери, м². Для
                   двупольной двери — площадь большей створки (п. 341).
    v_door_m_s   : v — нормативная скорость в проёме, м/с. По п. 340 — 1,3.
                   Для зон безопасности МГН по практике принимают 1,5.
    n_open_doors : n — число одновременно открытых дверей (расч. сценарий,
                   обычно 1 — дверь этажа пожара).

    Возвращает объёмный расход подпора, м³/ч (при t наружного воздуха).
    """
    if door_area_m2 <= 0 or v_door_m_s <= 0 or n_open_doors <= 0:
        return 0.0
    return 3600.0 * v_door_m_s * door_area_m2 * n_open_doors


def valve_leak_kg_h(valve_area_m2: float, dp_pa: float,
                    n_valves: int = 1) -> float:
    """ШНК 2.04.05-22, п. 327, ф.(5): подсос воздуха через закрытые дымовые
    клапаны системы дымоудаления (учитывается при подборе вентилятора СДУ):

        Gv = 40.3 · (Av · ΔP)^0.5 · n      [кг/ч]

    Av — площадь проходного сечения клапана, м²;
    ΔP — перепад давления на закрытом клапане, Па;
    n  — число закрытых клапанов в системе при пожаре.
    """
    if valve_area_m2 <= 0 or dp_pa <= 0 or n_valves <= 0:
        return 0.0
    return 40.3 * ((valve_area_m2 * dp_pa) ** 0.5) * n_valves
