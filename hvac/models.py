...
class BoundaryElement:
    ...
    # ===== Ручная правка элемента (is_exterior / конструкция / ориентация /
    # площадь) через панель «Ограждения». Когда True — правки сохраняются
    # в JSON как element_overrides и восстанавливаются поверх данных из CSV
    # (иначе в CSV-режиме ручные пометки «внутреннее/наружное» слетали при
    # перезагрузке проекта). См. io_json.SAVED_ELEMENT_FIELDS.
...
class Space:
    ...
    # ===== Зрители (спортзалы, залы со зрительскими местами) =====
    # ШНҚ 2.08.02-23 табл.23: приток = занимающиеся·норма + зрители·20 м³/ч.
    # occupancy_people — это занимающиеся; зрители считаются отдельно.
    spectator_count: int = 0        # число зрительских мест

    # ===== Парковки: расчёт по машино-местам (CO) =====
    # ШНҚ / СП 113: если задано (>0), вытяжка берёт max с расходом на машино-
    # места (m3_per_car из норм). 0 = расчёт по площади.
    car_count: int = 0              # число машино-мест

    # ===== Блок здания (раздел «Блоки») =====
    # Крупная часть комплекса, которой принадлежит помещение: "HTL" (отель),
    # "RES" (жильё), "OFF" (офис), "B1"/"B2" (подвалы), "GFL"/"MFL" (подиум).
    # Заполняется автоопределением (hvac/blocks.py: по префиксу номера, затем
    # по уровню) или вручную. Используется для раздельной работы и сводки
    # нагрузок по блокам (помещения + установки). Пусто = блок не определён.
    block: str = ""
    ...
    # ===== Раздельная привязка притока и вытяжки (опционально) =====
    # Обычно помещение обслуживает ОДНА система (system_ventilation), и на
    # неё относятся оба расхода. Если же помещение обслуживают ДВЕ независимые
    # установки (отдельная приточная + отдельный вытяжной вентилятор — техпо-
    # мещения: трансформаторные, котельные), поля ниже переопределяют, какой
    # системе принадлежит каждый расход:
    #   system_supply  — куда идёт приток (supply_m3h);
    #   system_exhaust — куда идёт вытяжка (exhaust_m3h) и зонт (hood_m3h).
    # Пустое поле = расход остаётся на system_ventilation. Раскраска планов и
    # прочая «основная» принадлежность помещения — по-прежнему по
    # system_ventilation. См. hvac/ahu_load.py (агрегация по атрибуции).
    system_supply: str = ""          # приточная система (переопределение)
    system_exhaust: str = ""         # вытяжная система (переопределение)
    ...
    # ===== Тепловой баланс: ручная классификация «отапливается/охлаждается» =====
    # Включает помещение в итоговую сумму нагрузок раздела «Тепловой баланс».
    # По умолчанию True — всё здание отапливается и охлаждается; пользователь
    # вручную снимает помещения, которые на деле не обслуживаются (склад без
    # охлаждения, паркинг без отопления и т.п.). Не влияет на расчёт нагрузок —
    # только на сводный итог. См. hvac/ui_qt/panels/balance_panel.py.
    is_heated: bool = True           # учитывать теплопотери в итоге отопления
    is_cooled: bool = True           # учитывать теплопоступления в итоге холода
    ...
    @property
    def vent_system_supply(self) -> str:
        """Система, которой принадлежит ПРИТОК помещения."""
        return self.system_supply or self.system_ventilation

    @property
    def vent_system_exhaust(self) -> str:
        """Система, которой принадлежит ВЫТЯЖКА и ЗОНТ помещения."""
        return self.system_exhaust or self.system_ventilation
...
class ProjectParameters:
    ...
    # Минимальная («фоновая») инфильтрация для помещений С механической
    # вентиляцией, ACH. По КМК 2.04.05 приточный воздух греет калорифер AHU
    # отдельно, а радиаторы догревают лишь подсос: при подпоре (приток ≥
    # вытяжки) инфильтрация ≈ этот фон; при преобладании вытяжки берётся
    # (вытяжка+зонт − приток). 0 — строго по подпору (пресс. помещение → 0);
    # 0.2–0.3 — оставить запас на открывание дверей / останов вентиляции.
    # Помещений БЕЗ механической вентиляции не касается (там воздухообмен по
    # ach_inf, естественный). См. hvac/engine/sp50.infiltration_flow_m3h.
    infiltration_min_ach: float = 0.0

    # Теплопотери/поступления через ВНУТРЕННИЕ перегородки к смежным
    # помещениям иной температуры (КМК 2.04.05-91: теплопередачу через внутр.
    # ограждения учитывают при разнице температур ≥ internal_partition_min_dt).
    # Сосед за стеной определяется по общему Revit-id стены (element_id);
    # площадь стены делится между всеми соседями по ней (точное разбиение по
    # сегментам — только из геометрии Revit). u_internal_partition — U внутр.
    # перегородки, Вт/(м²·К), когда у элемента не задан собственный U.
    internal_partition_min_dt: float = 3.0
    u_internal_partition: float = 1.5

    # Округление расчётных расходов вентиляции (приток/вытяжка/зонт) до
    # БЛИЖАЙШЕГО кратного этому шагу, м³/ч (.5 — вверх): 160.9→160, 166→170.
    # 0 — без округления. Типично 5 или 10. Не затрагивает помещения с ручным
    # расходом (vent_user_modified).
    round_airflow_m3h: float = 0.0
