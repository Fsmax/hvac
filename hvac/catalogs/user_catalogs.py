# -*- coding: utf-8 -*-
"""Пользовательские каталоги оборудования (радиаторы, фанкойлы, вентиляторы,
котлы, чиллеры).

Встроенные каталоги лежат в ``hvac/catalogs/data/radiators.json``,
``fancoils.json``, ``fans.json``, ``boilers.json`` и ``chillers.json``.
Дополнить их своими моделями можно без правки кода: положите JSON-файл
в ``~/.hvac_calc/catalogs/`` (имя любое, расширение ``.json``).
Каждый файл объявляет тип каталога:

    {
      "type": "radiators",            // или "fancoils", "fans",
                                      // "boilers", "chillers"
      "models": [ { ...поля модели... }, ... ],
...
Поля моделей совпадают с полями dataclass-моделей (``RadiatorModel`` /
``FancoilModel`` / ``FanModel`` / ``BoilerModel`` / ``ChillerModel``,
см. соответствующие ``*_catalog.py``); неизвестные ключи игнорируются,
...
def iter_user_catalogs(kind: str,
    ...
    kind     : "radiators" | "fancoils" | "fans" | "boilers" |
               "chillers" — отдаются только файлы, у которых
               совпало поле "type".
