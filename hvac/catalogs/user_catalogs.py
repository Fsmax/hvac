...
``fancoils.json`` и ``fans.json``. Дополнить их своими моделями можно
без правки кода: положите JSON-файл в ``~/.hvac_calc/catalogs/`` (имя
любое, расширение ``.json``). Каждый файл объявляет тип каталога:
...
(``RadiatorModel`` / ``FancoilModel`` / ``FanModel``); неизвестные ключи игнорируются,
...
def iter_user_catalogs(kind: str,
    ...
    kind     : "radiators" | "fancoils" | "fans" — отдаются только
               файлы, у которых совпало поле "type".
