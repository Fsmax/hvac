...
class TestWrappers:
    def test_import_from_revit(self, monkeypatch, tmp_path):
        calls = {}
        ...
        def fake_send(code, parameters=None, transaction_mode="auto",
            ...
            return {"spaces_rows": 10, "thermal_rows": 80, "orphan_rows": 2,
                    "source": "Spaces (MEP)",
                    "spaces_csv": str(tmp_path / "spaces.csv"),
                    "thermal_csv": str(tmp_path / "thermal_all.csv")}

        monkeypatch.setattr(revit_link, "send_code", fake_send)
        res = revit_link.import_from_revit(str(tmp_path))
        assert res["spaces_rows"] == 10
        assert calls["parameters"] == [str(tmp_path), "1"]
        assert calls["mode"] == "none"          # выгрузка без транзакции
        assert calls["code"] is revit_link.EXPORT_CS

    def test_import_no_orphans_flag(self, monkeypatch, tmp_path):
        seen = {}
        monkeypatch.setattr(
            revit_link, "send_code",
            lambda code, parameters=None, **kw: seen.update(p=parameters) or {})
        revit_link.import_from_revit(str(tmp_path), collect_orphans=False)
        assert seen["p"][1] == "0"
...
class TestCSharpTemplates:
    def test_braces_balanced(self):
        for cs in (revit_link.EXPORT_CS, revit_link.WRITEBACK_CS):
            assert cs.count("{") == cs.count("}"), "несбалансированные {}"
            assert cs.count("(") == cs.count(")"), "несбалансированные ()"
    ...
    def test_export_emits_loader_columns(self):
        ...
        cs = revit_link.EXPORT_CS
        for col in ("id", "category", "number", "name", "level",
                    "area", "volume", "height", "zone"):
            assert f'"{col}"' in cs, f"spaces.csv: нет колонки {col}"
        for col in ("space_id", "space_number", "space_name", "space_level",
                    "row_type", "is_exterior_wall", "element_id",
                    "link_model", "category", "family", "type",
                    "element_level", "boundary_length_m", "space_height_m",
                    "approx_area_m2", "element_area", "thickness", "function",
                    "thermal_value", "host_element_id",
                    "boundary_space_count", "orientation_deg"):
            assert f'"{col}"' in cs, f"thermal_all.csv: нет колонки {col}"

    def test_export_row_types_match_loader(self):
        """data_loader различает external_wall / opening — оба есть."""
        assert '"external_wall"' in revit_link.EXPORT_CS
        assert '"opening"' in revit_link.EXPORT_CS
        assert '"hosted by exterior wall"' in revit_link.EXPORT_CS
    ...
    def test_no_bare_io_or_newtonsoft_usings(self):
        ...
        for cs in (revit_link.EXPORT_CS, revit_link.WRITEBACK_CS):
            assert not re.search(r"(?<![\w.])File\.", cs), \
                "File. без префикса System.IO"
            assert not re.search(r"(?<![\w.])Directory\.", cs), \
                "Directory. без префикса System.IO"
            assert not re.search(r"(?<![\w.])JsonConvert\.", cs), \
                "JsonConvert. без префикса Newtonsoft.Json"
