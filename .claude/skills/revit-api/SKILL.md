...
`revitapidocs.com` и `rvtdocs.com` (2020–2027) — быстрый поиск по классам,
раздел `news` там же содержит список изменений по годам. Примеры Autodesk —
`jeremytammik/RevitSdkSamples`. Инспектировать живую модель удобнее всего
RevitLookup, а не отладчиком.
